...
function test_jac_terms()
  ...
  test_pressure(eqn.params, eqn.params_complex)
  test_pressure(eqn3.params, eqn3.params_complex)

  test_eulerflux(eqn.params, eqn.params_complex)
  test_eulerflux(eqn3.params, eqn3.params_complex)
  ...
  qg = q + 1
  test_ad_inner(eqn.params, eqn.params_complex, q, qg, nrm)

  println("testing all negative eigenvalues")
  q = Complex128[2.0, 3.0, 4.0, 7.0]
  qg = q + 1
  test_ad_inner(eqn.params, eqn.params_complex, q, qg, nrm2)
  ...
  qg = q + 1
  test_ad_inner(eqn3.params, eqn3.params_complex, q, qg, nrm)

  println("testing all negative eigenvalues")
  q = Complex128[2.0, 3.0, 4.0, 5.0, 13.0]
  qg = q + 1
  test_ad_inner(eqn3.params, eqn3.params_complex, q, qg, nrm2)
...
function test_pressure{Tdim}(params::AbstractParamType{Tdim}, paramsc)
  ...
  for j=1:numDofPerNode
    q[j] += pert
    p = EulerEquationMod.calcPressure(paramsc, q)
    p_dot[j] = imag(p)/h
    q[j] -= pert
  end
...
function test_eulerflux{Tdim}(params::AbstractParamType{Tdim}, paramsc)
  ...
  for j=1:numDofPerNode
    q[j] += pert
    EulerEquationMod.calcEulerFlux(paramsc, q, aux_vars, nrm, F)
...
function test_ad_inner{Tdim}(params::AbstractParamType{Tdim}, paramsc, qL, qR, nrm)
...
#    println("\nj = ", j)
    qL[j] += pert
    EulerEquationMod.RoeSolver(paramsc, qL, qR, aux_vars, nrm, F)
