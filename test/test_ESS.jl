...
#  println("qR = \n", qR)
  ...
  nrm = params.nrm
  E_full = zeros(sbpface.stencilsize, sbpface.stencilsize)
  ...
  for dim = 1:1  # DEBUGGING 1:TDIM
    ...
    for i = 1:sbpface.stencilsize
      ...
      for j = 1:sbpface.stencilsize
        ...
        F[:, p_i, p_j] = Flux_tmp[:]
        println("multiplying ",  Eij, " by ", Flux_tmp, " and storing in res ", p_i)
    ...
    println("after ij loop, resL[:, 1] = ", resL[:, 1])
    println("E_full = \n", E_full)
    for i=1:sbpface.stencilsize
      qi = sview(qL, :, i)
...
function getPsi{Tsol}(params, qL::AbstractMatrix{Tsol}, qR::AbstractMatrix{Tsol}, dir::Integer)
...
#  println("----- entered contractLHS -----")
  ...
  resR_vec = reshape(resR, length(resR))
  ...
  for i=1:numNodesPerElement
    ...
    w_tmp = sview(wR, :, i)
    EulerEquationMod.convertToEntropy_(eqn.params, qR_i, w_tmp)
    scale!(w_tmp, 1./params.gamma_1)
  end
...
#  println("resR = \n", resR)
  ...
  wR_vec = reshape(wR, length(wR))

  return dot(wL_vec, resL_vec), dot(wR_vec, resR_vec)
end
...
=#
  ...
  for i=1:mesh.numInterfaces
    ...
    rhsR = zero(Tres)
    for dir=1:1
      psiL, psiR = getPsi(eqn.params, qL, qR, dir)
      rhsL += sum(E_expensive*psiL)
      rhsR += -sum(E_expensive.'*psiR)
    end

    lhsL, lhsR = contractLHS(eqn.params, qL, qR, resL, resR)

    @fact lhsL + lhsR --> roughly(rhsL + rhsR, atol=1e-12)
