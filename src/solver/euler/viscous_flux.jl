...
function calcViscousFlux_interior{Tmsh, Tsol, Tres, Tdim}(mesh::AbstractDGMesh{Tmsh},
  ...
  for f = 1:nfaces    # loop over faces
    ...
    # TODO TODO remove after debugging!! viscous halt 20180409
    ...
    for n = 1:mesh.numNodesPerFace
      for iDof = 2 : mesh.numDofPerNode
        for jDof = 1 : mesh.numDofPerNode
          flux[iDof, n] +=  pMat[iDof, jDof, n]*dq[jDof, n]             # sign (+=) matches commit from Dec 2017
        end
      end
    end
    ...
    for n = 1:mesh.numNodesPerFace
      for iDof = 2 : mesh.numDofPerNode
        for iDim = 1 : Tdim
          flux[iDof, n] -= Fv_avg[iDim, iDof, n]*nrm_xy[iDim,n]         # sign (-=) matches commit from Dec 2017
        end
      end
    end
      ...
      for iDof = 2 : Tdim+2
        for iDim = 1 : Tdim
          if peeridx == 0       # TODO (speed) put if statement outside of loop
            ...
            eqn.vecflux_faceL[iDim, iDof, n, f] -=  HIGHLIGHTER*vecfluxL[iDim, iDof, n]*coef_nondim
            eqn.vecflux_faceR[iDim, iDof, n, f] -=  HIGHLIGHTER*vecfluxR[iDim, iDof, n]*coef_nondim
          else
            # AAAAA2: different array for shared case
            eqn.vecflux_faceL_shared[peeridx][iDim, iDof, n, f] -= HIGHLIGHTER*vecfluxL[iDim, iDof, n]*coef_nondim
          end   # end 'if peeridx == 0'
        end   # end 'for iDim = 1 : Tdim'
      ...
      if mesh.commsize == 1
        ...
        # if coordsX < xmax && coordsX > xmin
          # if coordsY < ymax && coordsY > ymin
            ...
            println(eqn.params.f, " flux_location[:,:,$f]: ", flux_location[:,:,f])
            println(eqn.params.f," mesh.coords_interface[1,:,$f]: ", mesh.coords_interface[1,:,f])
            println(eqn.params.f," mesh.coords_interface[2,:,$f]: ", mesh.coords_interface[2,:,f])
            println(eqn.params.f," mesh.interfaces[$f]: ", mesh.interfaces[f])
      # jacR = ro_sview(mesh.remote_metrics[peeridx].jac, :, elemR)
      # dxidxR = ro_sview(mesh.remote_metrics[peeridx].dxidx, :, :, :, elemR)
          # flux[iDof, n] -= Fv_avg[iDim, iDof, n]*nrm_xy[iDim,n]         # sign (-=) matches commit from Dec 2017
            println(eqn.params.f, " Fv_avg: ", Fv_avg)
            println(eqn.params.f, " nrm_xy: ", nrm_xy)
            println(eqn.params.f, " dq: ", dq)
          # end
        # end
        ...
        if peeridx == 0
          ...
          # if coordsX < xmax && coordsX > xmin
            ...
            # println(eqn.params.f," vecfluxR: ", vecfluxR)
              println(eqn.params.f, " flux_location[:,:,$f]: ", flux_location[:,:,f])
              println(eqn.params.f," mesh.coords_sharedface[peeridx][1,:,f]: ", mesh.coords_sharedface[peeridx][1,:,f])
              println(eqn.params.f," mesh.coords_sharedface[peeridx][2,:,f]: ", mesh.coords_sharedface[peeridx][2,:,f])
              println(eqn.params.f," mesh.shared_interfaces[$peeridx][$f]: ", mesh.shared_interfaces[peeridx][f])
              println(eqn.params.f, " Fv_avg: ", Fv_avg)
              println(eqn.params.f, " nrm_xy: ", nrm_xy)
              println(eqn.params.f, " dq: ", dq)
            # end
...
function evalFaceIntegrals_vector{Tmsh, Tsol, Tres, Tdim}(mesh::AbstractDGMesh{Tmsh},
  ...
  for f = 1 : nfaces
    ...
    if mesh.myrank == 0
      ...
      #=
      # MATCHES
      if mesh.commsize == 1 && f == 46
        print_now = 1
      end
      if mesh.commsize == 2 && peeridx == 0 && f == 17
        print_now = 2
      end
      # MATCHES
      if mesh.commsize == 1 && f == 54
        print_now = 1
      end
      if mesh.commsize == 2 && peeridx == 0 && f == 19
        print_now = 2
      end
      =#
      # on-shared
      #------------------------------------------------------------------------------
      if mesh.commsize == 1 && f == 52
        print_now = 1
      end
      if mesh.commsize == 2 && f == 4 && peeridx == 1
        print_now = 2
      end
      ...
      if print_now > 0 && peeridx == 0
        println(" mesh.coords_interface[:,:,$f]: ", round(mesh.coords_interface[:,:,f],3))
        # println(" eqn.vecflux_faceL[:,:,:,$f]: ", eqn.vecflux_faceL[:,:,:,f])
        # println(" RDxL: ", RDxL)
        println(" DxL: ", DxL)
        println(" R: ", R)
      end
      if print_now == 2 && peeridx == 1
        println(" mesh.coords_sharedface[1][:,:,$f]: ", round(mesh.coords_sharedface[1][:,:,f],3))
        # println(" eqn.vecflux_faceL_shared[1][:,:,:,$f]: ", eqn.vecflux_faceL_shared[1][:,:,:,f])
        # println(" RDxL: ", RDxL)
        println(" DxL: ", DxL)
        println(" R: ", R)
      end
