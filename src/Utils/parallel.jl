...
"""
  ...
  **Implementation Notes**
  ...
  all sends.
"""
function startSolutionExchange_rev(mesh::AbstractMesh, sbp::AbstractSBP,
                                  eqn::AbstractSolutionData, opts;
 ...
 if send_q
  ...
  elseif pdata == PARALLEL_DATA_ELEMENT
    populate_buffer = getSendDataElement_rev
  else
    throw(ErrorException("unsupported parallel_type = $(getParallelDataString(pdata))"))
  end
...
"""
  Like [`getSendDataElement`](@ref), but takes the data from `eqn.res_bar`
  and puts it in the send buffer.
"""
